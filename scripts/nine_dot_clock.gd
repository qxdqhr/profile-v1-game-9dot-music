...
## Audio-master clock: follows AudioStreamPlayer when a stream is set.
...
func resume() -> void:
	...
	_paused = false
	if _use_fallback:
		_fallback_start_usec = Time.get_ticks_usec() - (_frozen_raw_ms + _paused_accum_ms) * 1000
	if not _use_fallback and _player:
...
func _raw_ms_now() -> int:
	if _use_fallback or _player == null or not _player.playing:
		return int((Time.get_ticks_usec() - _fallback_start_usec) / 1000.0)
	return int(_player.get_playback_position() * 1000.0)
